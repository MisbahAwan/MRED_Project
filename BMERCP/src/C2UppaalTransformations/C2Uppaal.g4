...
file: (preprocessorDirective | includeDirective | moduleDecl  )* EOF ;
...
parameterList: dataType CONST? STAR? ID? (COMMA? dataType CONST? STAR? ID?)* ;

//parameter: ID | 'void' | 'const' |dataType;

declaration: EXTERN? CONST? dataType ID (LBRACK INT RBRACK)? (EQUAL primary)? SEMICOLON ;
...
//ifStatement: (IF LPAREN expression RPAREN ((LBRACE statement+ RBRACE) | statement) comment?) (ELSE (IF LPAREN expression RPAREN)? ((LBRACE statementTwo+ RBRACE) | statementTwo) )* ;
...
caseBlock: CASE expression COLON LBRACE? statement* RBRACE? BREAK SEMICOLON? RBRACE?;
