...
CONST: 'const';
VOID: 'void';
DATATYPE: 'int' |  'double' | 'char' | 'string' |'clock' | 'real'; // Data types
...
expression:  ( ID  
              ...
              (LBRACK INT RBRACK)?  
              (EQEQ|EQUAL| GE| AND |LT_TAG | GT_TAG | PLUSPLUS | MINUSMINUS | MINUSEQUAL | PLUSEQUAL | EQUALMINUS | EQUALPLUS | LPAREN ((HEX_INT| (ID MINUS LPAREN ID MINUS ID RPAREN) | ID |INT) COMMA?)*? RPAREN|NOTEQ |PLUSPLUS ) ?
              ((LPAREN? ID (PERCENT|SLASH) INT RPAREN? SLASH? INT? PLUS STRING ) | ( (ID|EQUAL|STRING) )|(ID PLUS INT)|ID | INT |ID LPAREN ID? RPAREN)? COMMA? | comment | (ID operator ID COMMA?))*? ;
